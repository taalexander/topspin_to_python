...
class FID(object):
	...
	def ft(self,phase=0):
		...
		return FT(ft,freqs_shifted,phase=phase,sfo=self.sfo)
	...
	def __unicode__(self):
		return {'fid':self.fid,'times':self.times}.__unicode__()[1:-1]
...
class FT(object):
	...
	def apk(self,method=None,use_lorentzian=False,left=None,right=None,ppm=False,**opt_pars):
		"""
		...
		res = opt.minimize(min_func,[0],method=method,**opt_pars)
	...
	def __unicode__(self):
		return {'ft':self.ft,'freqs':self.freqs,'phase':self.phase,'sfo':self.sfo}.__unicode__()[1:-1]